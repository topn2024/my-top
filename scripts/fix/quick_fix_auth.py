...
def main():
    ...
    # 上传auth.py
    sftp = ssh.open_sftp()
    local_file = PROJECT_ROOT / "backend" / "auth.py"
    remote_file = f"{DEPLOY_DIR}/backend/auth.py"
    ...
    # 测试
    print("\n测试...")
    stdin, stdout, stderr = ssh.exec_command("curl -s http://localhost:3001/api/health | head -1")
    print(stdout.read().decode('utf-8', errors='ignore'))
